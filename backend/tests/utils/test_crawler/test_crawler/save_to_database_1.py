        ...
        """,
            {
                ...
                if is_float(record.get(f"{prefix}_Main_Flow", {}).get("Net_Percentage", ""))
                else None,
                "Extra_Large_Order_Flow_Net_Amount": float(
                    record.get(f"{prefix}_Extra_Large_Order_Flow", {}).get("Net_Amount", None)
                )
                if is_float(
                    record.get(f"{prefix}_Extra_Large_Order_Flow", {}).get("Net_Amount", "")
                )
                else None,
                "Extra_Large_Order_Flow_Net_Percentage": float(
                    record.get(f"{prefix}_Extra_Large_Order_Flow", {}).get("Net_Percentage", None)
                )
                if is_float(
                    record.get(f"{prefix}_Extra_Large_Order_Flow", {}).get("Net_Percentage", "")
                )
                else None,
                "Large_Order_Flow_Net_Amount": float(
                    record.get(f"{prefix}_Large_Order_Flow", {}).get("Net_Amount", None)
                )
                if is_float(record.get(f"{prefix}_Large_Order_Flow", {}).get("Net_Amount", ""))
                else None,
                "Large_Order_Flow_Net_Percentage": float(
                    record.get(f"{prefix}_Large_Order_Flow", {}).get("Net_Percentage", None)
                )
                if is_float(record.get(f"{prefix}_Large_Order_Flow", {}).get("Net_Percentage", ""))
                else None,
                "Medium_Order_Flow_Net_Amount": float(
                    record.get(f"{prefix}_Medium_Order_Flow", {}).get("Net_Amount", None)
                )
                if is_float(record.get(f"{prefix}_Medium_Order_Flow", {}).get("Net_Amount", ""))
                else None,
                "Medium_Order_Flow_Net_Percentage": float(
                    record.get(f"{prefix}_Medium_Order_Flow", {}).get("Net_Percentage", None)
                )
                if is_float(record.get(f"{prefix}_Medium_Order_Flow", {}).get("Net_Percentage", ""))
                else None,
                "Small_Order_Flow_Net_Amount": float(
                    record.get(f"{prefix}_Small_Order_Flow", {}).get("Net_Amount", None)
                )
                if is_float(record.get(f"{prefix}_Small_Order_Flow", {}).get("Net_Amount", ""))
                else None,
                "Small_Order_Flow_Net_Percentage": float(
                    record.get(f"{prefix}_Small_Order_Flow", {}).get("Net_Percentage", None)
                )
                if is_float(record.get(f"{prefix}_Small_Order_Flow", {}).get("Net_Percentage", ""))
                else None,
            },
        )
