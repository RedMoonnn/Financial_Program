...
# 异步可视化数据
async def visualize_data(title, day, page):
    # 设置中文字体
    ...
    json_loaded = await read_json_file(json_file_path)
    ...
    # 直接调用主线程中的绘图函数
    plot_and_save(title, time_text, x_positions, names, prices, page)

# 绘图和保存图表的函数，确保在主线程中执行
def plot_and_save(title, time_text, x_positions, names, prices, page):
    # 可视化
