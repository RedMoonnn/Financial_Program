...
from save_to_database_2 import *
from source_data_2 import *
from visualize_2 import *

# 捕获并忽略 aiomysql 模块的所有 UserWarning 类型警告
...
# 异步调用两个函数
async def main(data, title, pages, day_name, db_config):
    # 使用 asyncio.gather 并发调用
    await asyncio.gather(
        visualize_data(title, day_name, pages),  # 等待可视化函数完成
        store_data_to_db(data, title, day_name, db_config)  # 等待数据库存储函数完成
    )
...
# 执行异步主程序
asyncio.run(main(data, title, pages, day_name, db_config))
